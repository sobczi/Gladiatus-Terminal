    ...
    return login[0], password[0], server[0], expedition_enabled[0],\
         expedition_option[0], pack_gold_enabled[0], val_pack_gold[0],\
              dungeon_enabled[0], dungeon_option[0], health[0],\
                   backpack[0], backpack_food[0], headless[0]
...
def filter_packages(first, second):
    type_package = type_pack(first)
    quality_package = quality_pack(second)
    click_element("//select[@name = 'f']//option[text() = '" + str(type_package) + "']")
    click_element("//select[@name = 'fq']//option[text() = '" + str(quality_package) + "']")
...
def login():
    ...
    click_element("//option[@value='s"+str(server)+"-pl.gladiatus.gameforge.com/game/index.php?mod=start&submod=login']")
...
def heal_me():
    while int(get_hp_value()) < int(health):
        ...
        drag_and_drop("//div[@id='inv']//div[@data-content-type-accept='16777215']","//div[@id='avatar']//div[@class='ui-droppable']")
        time.sleep(2)
    return    
...
def prepare_xpath(name,soulbound,level,quality,amount):
    ...
    if name != 'None':
        ...
        path2 = path2 + "[contains(concat(' ', normalize-space(@class), ' '), ' " + name + " ')]"
...
def download_packages():
    ...
    for i in range(2, int(iterator)):
        element = driver.find_element_by_xpath("//section[@id='market_table']//tr[position()='" + str(i) + "']/td[@style]/div[@style]")
        soulbound = element.get_attribute('data-soulbound-to')
        price = driver.find_element_by_xpath("//section[@id='market_table']//tr[position()='" + str(i) + "']/td[position()='3']").text
        price = price.replace(".","")
        ...
        already_sold = 'False'
        temp = driver.find_element_by_xpath("//section[@id='market_table']//tr[position()='" + str(i) + "']/td[@style]/div[@style]")
        action.move_to_element(temp).perform()
        if search_element("//p[contains(text(),'Wskazówka')]"):
            already_sold = 'True'
        
        ready_line = "class_name='" + str(class_name) + "' soulbound='" + str(soulbound) + "' price='" + str(price) + "' category='" + str(category) + "' quality='" + str(quality) + "' level='" + str(level) + "' amount='" + str(amount) + "' sold='" + str(already_sold) + "'\n"
...
def expedition():
    ...
    temp2 = driver.find_element_by_xpath("//div[@class='expedition_box']["+expedition_option+"]//div[@class='expedition_name']").text
...
def dungeon():
    ...
    if search_element("//div[@id='cooldown_bar_dungeon']/a[@class='cooldown_bar_link']"):
        element = driver.find_element_by_xpath("//div[@id='cooldown_bar_dungeon']/a[@class='cooldown_bar_link']")
    ...
    wait_for_element("//div[@id='cooldown_bar_dungeon']/div[@id='cooldown_bar_text_dungeon']")
...
def pack_gold():
    ...
    while not bought:
        ...
        if name_temp != 'None':
            by_name = True
        if soulbound_temp != 'None':
            by_soulbound = True
        if level_temp != 'None':
            by_level = True
        if quality_temp != 'None':
            by_quality = True
        ...
        for i in range(2, int(iterator)+1):
            if search_element("//section[@id='market_table']//tr[position()='" + str(i) + "']/td[@align='center']/input[@value='Kup']"):
                ...
                if price_temp == price:
                    ...
                    driver.find_element_by_xpath("//section[@id='market_table']//tr[position()='" + str(i) + "']/td[@align='center']/input[@value='Kup']").click()
    ...
    path, path2 = prepare_xpath(classes[found_case],soulbound_temp,level_temp,quality_temp,amount_temp)
    #gib it back niggur 
        ...
        while not both_locations:
            ...
            for item in items:
                ...
                if by_name and by_level and by_soulbound and by_quality and by_amount and by_sold:
                    path1, path2 = prepare_xpath(classes[i],soulbounds[i],levels[i],qualities[i],amounts[i])
                    found_sold = solds[i]
...
def take_hades_costume():
    ...
    if search_element("//input[contains(@onclick,'Zbroja Disa Patera')]"):
        ...
        click_element("//td[@id='buttonleftchangeCostume']/input[@value='Tak']")
        return True
    else:
        return False
    return False
    ...
    username, password, server, expedition_enabled, expedition_option,\
        pack_gold_enabled, pack_gold_minimum, dungeon_enabled,\
            dungeon_advenced, health, backpack_free,\
                backpack_food, headless = read_settings(sys.argv[1])
except:
